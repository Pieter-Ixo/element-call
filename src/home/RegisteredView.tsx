...
export const RegisteredView: FC<Props> = ({ client }) => {
  ...
  // IXO: when only a single recent group call room exists, navigate straight
  // into it. The mobile WebView always opens to a specific call so we don't
  // want the user to land on the home screen and tap through.
  useEffect(() => {
    if (recentRooms.length === 1) {
      ...
      void navigate(
        getRelativeRoomUrl(room.roomId, encryptionSystem, room.name),
      );
    }
